...
[打开 Netlify 在线演示](https://mosquito-risk-demo-2.netlify.app/)

该链接用于展示工作台的页面设计与完整交互流程，使用预置演示数据。它不连接本项目的 FastAPI 后端、Worker、SQLite 数据库或 YOLO 模型，不代表真实图像推理与报告生成服务已经部署到云端。完整功能请在已配置本地环境和模型权重的 Windows 电脑上运行。
